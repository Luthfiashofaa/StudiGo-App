...
serve(async (req) => {
  try {
    ...
    // Accept both `email` and fallback to `name` for quick tests
    ...
    // For simplicity this example does not store codes. Add storage logic if needed.

    // Try to send with SendGrid if configured
    ...
    if (devReturn) {
      resp.dev_code = code;
      // expose sendResult in dev mode to help debugging why emails aren't delivered
